...
8. `/discover` must never show an empty screen: vendors outside the budget band still show, labeled "above"/"below your budget"; cuisine and event-type preference are soft ranking signals, not hard filters — a vendor matching neither still shows, under a subtle "Also available" divider (see `matchesFilters` in `lib/matching.ts`; matches on cuisine OR event type, not both required). Both `DiscoverableVendor` and the discover query must actually select `cuisine_specialities`/`event_specialities` for this to work — this broke silently once before (UI state existed, `MatchCriteria` had no `eventTypes` field, and the query didn't even fetch the column), so when adding a new discovery filter, trace it all the way from the dropdown's `onChange` through `MatchCriteria` into `matchVendors`, and confirm the query selects whatever column it reads.
